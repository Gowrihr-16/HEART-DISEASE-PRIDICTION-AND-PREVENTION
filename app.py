...
def add_bg_from_local(image_file):
    ...
    st.markdown(
        ...
        <style>
        .stApp {{
            background-image: url("data:image/jpg;base64,{encoded_string.encode('base64').decode()}");
...
# Alternate approach (recommended for simplicity)
import base64
...
# --- Add Background ---
# Place your image (e.g., 'bg.jpg') inside your project folder
# and make sure to give the correct path here.
set_background("bg1.jpg")  # Replace 'bg.jpg' with your image filename
...
def main():
    ...
    # Welcome page
    if not st.session_state.welcome_shown:
        st.write("This application helps you predict the risk of heart disease based on several health metrics.")
        ...
        if choice == "Login":
            ...
            if st.button("Register"):
                create_user(new_user, new_password, email)
                st.success("User created successfully!")
...
""")
    ...
    thalach = st.number_input("Maximum Heart Rate Achieved", min_value=60, max_value=220)
    exang = st.selectbox("Exercise Induced Angina", [0, 1])
    oldpeak = st.number_input("Oldpeak", min_value=0.0, max_value=6.0)
    slope = st.selectbox("Slope of the Peak Exercise ST Segment", [0, 1, 2])
    ca = st.selectbox("Number of Major Vessels (0-3)", [0, 1, 2, 3])
    thal = st.selectbox("Thalassemia", [0, 1, 2, 3])
    ...
    if st.button("Predict"):
        ...
        precautions = [
            ...
            "Avoid smoking and excessive alcohol consumption.",
            "Regular check-ups with your healthcare provider.",
            "Control your cholesterol, blood glucose, and blood pressure.",
            "Drink alcohol only in moderation.",
            "Get enough sleep."
        ]
        st.subheader("Precautions:")
